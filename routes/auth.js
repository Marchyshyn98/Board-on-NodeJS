...
router.post("/register", (req, res) => {
    ...
    if (!login || !password || !passwordConfirm) {
    ...
    } else if (!/$/.test(login)) { // регулярне вираження для тесту на букви малі, великі і цифри...якщо не співпадає
        res.json({
            ok: false,
            error: "Всі букви повинні бути малими",
            fields: ['login']
        });
        ...
        res.json({
            ok: false,
            error: "Довжина паролю не менше 5",
            fields: ['password']
        });
    } else {
